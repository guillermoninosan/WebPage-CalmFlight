...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 w-full z-50 bg-[#050a14]/90 backdrop-blur-xl border-b border-white/5">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex justify-between items-center relative">

                <Link href="/" className="flex items-center gap-1.5 hover:opacity-80 transition-opacity z-10 shrink-0">
                    <Image src="/logo_wide.webp" alt="Logo" width={32} height={32} className="h-7 w-auto sm:h-8 rounded-lg shadow-lg" unoptimized />
                    <span className="font-bold text-xl sm:text-2xl tracking-tight text-white leading-none pt-0.5">CalmFlight</span>
                </Link>
                ...
                <div className="flex gap-2 sm:gap-4 items-center relative shrink-0">
                    ...
                    <div className="relative" ref={dropdownRef}>
                        <button
                            ...
                            className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-2 sm:px-3 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all w-auto sm:w-32 justify-between"
                        >
                            <span className="truncate max-w-[60px] sm:max-w-none">{currentLabel}</span>
